 ...
 */
function testThemeContrast(themeName: string, themeType: 'color' | 'vivid' | 'pastel'): void {
  logger.info(`Testing contrast improvement for theme: ${themeName} (${themeType})`);
  ...
  const enhancedContrastRatio = getContrastRatio(enhancedCommentColor, backgroundColor);
  logger.info(`Enhanced contrast ratio: ${enhancedContrastRatio.toFixed(2)}`);

  // Calculate improvement percentage
  const improvementPercentage =
    ((enhancedContrastRatio - originalContrastRatio) / originalContrastRatio) * 100;
